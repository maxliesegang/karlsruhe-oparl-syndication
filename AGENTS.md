...
- When re-tuning, measure against `docs/` before and after. `tests/karlsruhe-districts.test.ts` pins the matcher and the grading rules; `tests/paper-district-index-service.test.ts` pins the index shape and the incremental paths.
