...
- **A response the SDK cannot parse is salvaged, not re-requested.** `Output.json()` rejects outright when the model wraps its JSON in a `<think>` preamble or a code fence, which is what “No object generated: could not parse the response” means. `salvageJsonObject` strips those and extracts the balanced object from the rejected text; a response truncated by `MAX_OUTPUT_TOKENS` yields no balanced object and stays a failure. That token budget (1600) is mostly headroom for the reasoning preamble, which counts against it — only generated tokens are billed, so the slack is free.
