...
- Summaries are content-addressed using a SHA-256 hash of the exact semantic input sent to the model: relevant paper metadata, the public consultation/result timeline, attachment labels, and current extracted text. Generic OParl `modified` timestamps and array order are excluded, so timestamp-only refreshes and reorder-only updates never call the model again; a changed published result does. Canonical per-paper records live under `docs/summaries/papers/`.
- The public consultation/result timeline is repeated for every raw-text chunk and every synthesis pass. The feed also renders each agenda item's exact OParl `result` as a deterministic “Beratungsstand dieser Sitzung”, so a provider failure cannot hide a newly arrived result.
